...
def main():
    df = pd.read_csv('data/seoul_art100.csv')
    ...
    map_osm = folium.Map(location = [x[15],y[15]],zoom_start=14)

    for i in range(len(x)) :
        folium.Marker([x[i],y[i]], popup=df.iloc[i,1:3].values, icon=folium.Icon(color='red',icon='info-sign')).add_to(map_osm)

    folium.CircleMarker(location=[x[15],y[15]],popup='주소',radius = 300,color="#3186cc", fill_color="#3186cc").add_to(map_osm)
